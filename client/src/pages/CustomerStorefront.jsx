    ...
    <div className="product-tile flex flex-col relative group">
      ...
      <div className="p-3 flex flex-col gap-2 flex-1">
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          <button
            onClick={handleAdd}
            className={`flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${adding || isAdded
  ...
  const fetchCatalog = async () => {
    try {
      ...
      if (discountedItems.length > 0) {
        greeting += `\n\n🔥 **Active Deals Live Now**: We have special discounts running today! Ask me for recommendations or deals anytime. 🛍️`;
      } else {
        greeting += `\n\nAsk me for recommendations, search for products, or discover active deals. 🛍️`;
      }

      setMessages([{
        role: 'assistant',
        content: greeting,
        action: discountedItems.length > 0 ? {
          intent: 'INFO',
          recommendedSkus: discountedItems.slice(0, 3).map(d => d.sku)
        } : null,
    ...
    return (
      <div className="flex items-center gap-2.5 p-2.5 rounded-xl bg-violet-950/60 border border-violet-800/40 hover:border-violet-500/40 transition-all">
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[10px] font-bold transition-all cursor-pointer ${isAdded
              ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
            ...
            return (
              <div key={idx} className={`flex items-end gap-2.5 ${isAgent ? 'justify-start' : 'justify-end'} animate-fade-in-up`}>
                ...
                <div className={`max-w-[85%] ${isAgent ? 'space-y-2' : ''}`}>
                  <div className={`px-4 py-3 text-xs leading-relaxed ${isAgent ? 'chat-bubble-agent text-slate-200' : 'chat-bubble-user text-white shadow-lg shadow-violet-700/20'
                    }`}>
                    <div className="leading-relaxed space-y-1 [&_p]:my-1 [&_p:first-child]:mt-0 [&_p:last-child]:mb-0 [&_ul]:my-1.5 [&_ul]:pl-4 [&_ul]:list-disc [&_ol]:my-1.5 [&_ol]:pl-4 [&_ol]:list-decimal [&_li]:my-0.5 [&_strong]:text-white [&_strong]:font-bold [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:rounded [&_code]:bg-violet-950/80 [&_code]:border [&_code]:border-violet-700/40 [&_code]:text-amber-300 [&_code]:font-mono [&_code]:text-[11px]">
        ...
        {/* Suggested prompts */}
        <div className="px-4 py-2 border-t border-violet-900/20 bg-[#070510]/40 overflow-x-auto flex gap-2 no-scrollbar">
          {QUICK_PROMPTS.map((prompt, i) => (
            <button
              key={i}
              onClick={() => setInputMessage(prompt)}
              className="shrink-0 px-3 py-1.5 rounded-lg bg-violet-950/60 hover:bg-violet-900/60 border border-violet-800/40 text-[11px] text-violet-300 hover:text-white transition-all cursor-pointer"
            >
              {prompt}
            </button>
          ))}
        ...
        {/* Message input */}
        <form onSubmit={handleSendMessage} className="p-3 border-t border-violet-900/20 bg-[#070510]/80 backdrop-blur">
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="Ask AI agent to find products, apply coupons, or help checkout..."
              value={inputMessage}
              onChange={e => setInputMessage(e.target.value)}
              disabled={loadingChat}
              className="flex-1 px-4 py-2.5 rounded-xl bg-violet-950/40 border border-violet-800/40 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-violet-500 focus:bg-violet-950/60 transition-all"
            />
            <button
              type="submit"
              disabled={loadingChat || !inputMessage.trim()}
              className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 disabled:opacity-40 text-white text-xs font-bold flex items-center gap-1.5 shadow-lg shadow-violet-600/30 transition-all cursor-pointer shrink-0"
            >
      ...
      <div className="w-80 xl:w-96 shrink-0 flex flex-col border-l border-violet-900/20 bg-[#06040f]/60">
        <div className="p-4 border-b border-violet-900/20 flex items-center justify-between">
          ...
          <button
            ...
            className="text-[11px] text-slate-500 hover:text-red-400 transition-colors disabled:opacity-30 flex items-center gap-1"
          >
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-3">
          ...
          {cart.length === 0 ? (
            <div className="py-10 text-center text-xs text-slate-600">
              ...
              Your cart is empty.<br />Ask the AI agent to find products for you!
            </div>
          ) : (
            <>
          ...
          {upsellData?.recommendations?.length > 0 && (
            <div className="space-y-2">
              ...
              {upsellData.recommendations.map(rec => (
                <div
                  key={rec.sku}
                  className={`p-2.5 rounded-xl border transition-all ${rec.type === 'UPSELL'
                ...
                >
                  <div className="flex items-start justify-between gap-1 mb-1">
                    <div>
                      {rec.type === 'UPSELL' && (
                        <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300">
                          ⚡ UPSELL
                        </span>
                      )}
                      <div className={`text-xs font-bold text-white leading-tight ${rec.type === 'UPSELL' ? 'mt-1' : ''}`}>{rec.name}</div>
                    </div>
